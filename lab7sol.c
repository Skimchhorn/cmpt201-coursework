...
void groupByKey(IntermediateInput *input, Output *output, int *result_count) {
  ...
  // New value
  Output *newOutput = &output[*result_count];
  newOutput->doubled_value = input->doubled_value;
  newOutput->line_numbers[0] = input->line_number;
  newOutput->count = 1;
  (*result_count)++;
}
...
void reduce(Output *output) {
  ...
  for (int i = 0; i < output->count; i++) {
    printf("%d%s", output->line_numbers[i], i < output->count - 1 ? ", " : "");
  }
  printf("])\n");
}
